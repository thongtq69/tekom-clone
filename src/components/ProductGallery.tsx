 ...
 * Hover = zoom in. Click = open lightbox (does NOT navigate).
 */
  ...
  return (
    <>
      <div className="relative border-t border-[color:var(--color-line)]">
        ...
        >
          {shown.map(({ src, idx }) => (
            <button
              ...
              className="relative aspect-[3/4] overflow-hidden group cursor-zoom-in focus:outline-none focus:ring-2 focus:ring-[color:var(--color-gold)]"
              aria-label={`${alt} ${idx + 1}`}
            >
              <div
                className="absolute inset-0 bg-cover bg-center transition-transform duration-300 ease-out group-hover:scale-150"
        ...
        {hoverIdx !== null && (
          <div
            className="fixed inset-0 z-[90] pointer-events-none flex items-center justify-center bg-black/75 backdrop-blur-sm p-6 md:p-10"
            aria-hidden
          >
